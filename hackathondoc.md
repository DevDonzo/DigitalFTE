...
Basic folder structure: /Needs_Action, /Inbox (legacy), /Done
...
Create accounting system for your business in Xero ( https://www.xero.com/ ) and integrate it with its MCP Server ( https://github.com/XeroAPI/xero-mcp-server )
...
'''
        ...
        return filepath
WhatsApp Watcher (Twilio webhook-based)
Note: This uses Twilio's WhatsApp webhook. No browser automation required.
# webhook_server.py (ingest) + whatsapp_watcher.py (process)
...
app = FastAPI()
vault = Path("./vault")
incoming_store = vault / ".whatsapp_incoming.json"
...
async def receive_webhook(request: Request):
    ...
    payload = {"id": msg_id, "from": from_number, "text": {"body": text}}
    data = {"messages": []}
    if incoming_store.exists():
        data = json.loads(incoming_store.read_text())
    data["messages"].append(payload)
    incoming_store.write_text(json.dumps(data, indent=2))
    return ""
...
Browser MCP (optional): Placeholder if you later add browser automation for payment portals.
...
Python watchdog script monitoring the /Needs_Action folder (or /Inbox legacy).
Project-Based
...
Browser MCP placeholder for future "Computer Use" automation (optional).
...
www.twilio.com/docs/whatsapp/tutorial/connect-number-business-profile
Docs
